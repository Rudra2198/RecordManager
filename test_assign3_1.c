...
main (void)
{
    ...
    testInsertManyRecords(); // current issue, page directory info missing after close table
    testRecords();
...
testRecords (void)
{
    ...
    tmpValue = stringToValue("i1");
    OP_TRUE(tmpValue, value, valueEquals, "first attr");
    freeVal(tmpValue);
    freeVal(value);

    getAttr(r, schema, 1, &value);
    tmpValue = stringToValue("saaaa");
    OP_TRUE(tmpValue, value, valueEquals, "second attr");
    freeVal(tmpValue);
    freeVal(value);

    getAttr(r, schema, 2, &value);
    tmpValue = stringToValue("i3");
    OP_TRUE(tmpValue, value, valueEquals, "third attr");
    freeVal(tmpValue);
    freeVal(value);

    // modify attrs
    tmpValue = stringToValue("i4");
    setAttr(r, schema, 2, tmpValue);
    freeVal(tmpValue);

    getAttr(r, schema, 2, &value);
    tmpValue = stringToValue("i4");
    OP_TRUE(tmpValue, value, valueEquals, "third attr after setting");
    freeVal(tmpValue);
    freeVal(value);
...
testCreateTableAndInsert (void)
{
    ...
    Record *insertedRecords[numInserts];
    ...
    for(i = 0; i < 1000; i++)
    {
        ...
        Record *updatedRecord = fromTestRecord(schema, inserts[pos]);
        ASSERT_EQUALS_RECORDS(updatedRecord, r, schema, "compare records");
        freeRecord(updatedRecord); // Free the updated record object
        //ASSERT_EQUALS_RECORDS(fromTestRecord(schema, inserts[pos]), r, schema, "compare records");

    }
    // Free the Record objects created by fromTestRecord
    for (i = 0; i < numInserts; i++)
    {
        if (insertedRecords[i] != NULL)
        {
            freeRecord(insertedRecords[i]);
            insertedRecords[i] = NULL;
        }
    }
...
testUpdateTable (void)
{
    ...
    rids = (RID *) malloc(sizeof(RID) * numInserts);
    Record *insertedRecords[numInserts];
    Record *insertedRecords1[numInserts];
    ...
    for(i = 0; i < numFinal; i++)
    {
        RID rid = rids[i];
        TEST_CHECK(getRecord(table, rid, r));
        Record *updatedRecord = fromTestRecord(schema, finalR[i]);
        ASSERT_EQUALS_RECORDS(updatedRecord, r, schema, "compare records");
        freeRecord(updatedRecord); // Free the updated record object
    ...
    for (i = 0; i < numUpdates; i++)
    {
        if (insertedRecords1[i] != NULL)
        {
            freeRecord(insertedRecords1[i]);
            insertedRecords1[i] = NULL;
        }
    }
...
testInsertManyRecords(void)
{
    ...
    for(i = 0; i < numInserts; i++)
    {
        RID rid = rids[i];
        TEST_CHECK(getRecord(table, rid, r));
        Record *expectedRecord = fromTestRecord(schema, realInserts[i]);
        ASSERT_EQUALS_RECORDS(expectedRecord, r, schema, "compare records");
        //ASSERT_EQUALS_RECORDS(fromTestRecord(schema, realInserts[i]), r, schema, "compare records");
        freeRecord(expectedRecord); // Free the expected record object
    }
    ...
    Record *updatedRecord = fromTestRecord(schema, updates[0]);
    ASSERT_EQUALS_RECORDS(updatedRecord, r, schema, "compare records");
    freeRecord(updatedRecord); // Free the updated record object
...
void testScans (void)
{
    ...
    while((rc = next(sc, r)) == RC_OK)
    {
        ...
        {
            Record *updatedRecord = fromTestRecord(schema, scanOneResult[i]);
            if (memcmp(updatedRecord->data,r->data,getRecordSize(schema)) == 0)
            {
                foundScan[i] = TRUE;
                freeRecord(updatedRecord); // Free the updated record object
...
void testScansTwo (void)
{
    ...
    RID *rids;
    Schema *schema;
    Value *tmpValue, *tmpValue1, *tmpValue2;
    RM_ScanHandle *sc = (RM_ScanHandle *) malloc(sizeof(RM_ScanHandle));
    Expr *sel1, *sel, *left1, *right1, *left, *right, *first, *se;
    int rc;
    ...
    tmpValue = stringToValue("i2");
    MAKE_CONS(left1, tmpValue);
    //MAKE_CONS(left, stringToValue("i2"));
    MAKE_ATTRREF(right1, 0);
    MAKE_BINOP_EXPR(sel1, left1, right1, OP_COMP_EQUAL);
    createRecord(&storedRecords[0], schema);
    TEST_CHECK(startScan(table, sc, sel1));
    while((rc = next(sc, storedRecords[0])) == RC_OK)
    {
        Record *updatedRecord = fromTestRecord(schema, inserts[1]);
        ASSERT_EQUALS_RECORDS(updatedRecord, storedRecords[0], schema, "compare records");
        freeRecord(updatedRecord); // Free the updated record object
    ...
    tmpValue1 = stringToValue("sffff");
    MAKE_CONS(left, tmpValue1);
    //MAKE_CONS(left, stringToValue("sffff"));
    MAKE_ATTRREF(right, 1);
    MAKE_BINOP_EXPR(sel, left, right, OP_COMP_EQUAL);
    createRecord(&storedRecords[1], schema);
    TEST_CHECK(startScan(table, sc, sel));
    while((rc = next(sc, storedRecords[1])) == RC_OK)
    {
        Record *updatedRecord = fromTestRecord(schema, inserts[5]);
        ASSERT_EQUALS_RECORDS(updatedRecord, storedRecords[1], schema, "compare records");
        //ASSERT_EQUALS_RECORDS(fromTestRecord(schema, inserts[5]), r, schema, "compare records");
        char *serializedRecord = serializeRecord(storedRecords[1], schema);
    ...
    tmpValue2 = stringToValue("i4");
    MAKE_CONS(left, tmpValue2);
    ...
    while((rc = next(sc, storedRecords[0])) == RC_OK)
    {
        char *serializedRecord = serializeRecord(storedRecords[0], schema);
        free(serializedRecord);
        for(i = 0; i < numInserts; i++)
        {
            Record *updatedRecord = fromTestRecord(schema, inserts[i]);
            if (memcmp(updatedRecord->data,storedRecords[0]->data,getRecordSize(schema)) == 0)
                foundScan[i] = TRUE;
            freeRecord(updatedRecord); // Free the updated record object
        }
    }
    ...
    freeRecord(storedRecords[0]);
    freeRecord(storedRecords[1]);
    freeSchema(schema);
    free(rids);
    free(table);
    free(sc);
